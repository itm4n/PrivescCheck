...
"APP_ROOTDRIVE",                  "Invoke-ProgramsInDriveRootCheck",            "TA0004 - Privilege Escalation", "Root folder permissions",             "Low",      "List",   "False",    "False",      "True",         "Check for custom folders in fixed drive roots that inherit insecure permissions by default."
...
"CREDS_SCCM_NAA",                 "Invoke-SccmNaaCredentialsCheck",             "TA0006 - Credential Access",    "SCCM Network Access Account (NAA)",   "Medium",   "List",   "False",    "True",       "False",        "Check whether SCCM NAA credentials are stored in the WMI repository. If so, the username and password DPAPI blobs are returned, but can only be decrypted using the SYSTEM's DPAPI user key."
"CREDS_SCCM_CACHE_FOLDERS",       "Invoke-SccmCacheFolderCredentialsCheck",     "TA0006 - Credential Access",    "SCCM cache folders (credentials)",    "Low",      "List",   "True",     "True",       "False",        "Check whether the SCCM cache folders contain files with potentially hard coded credentials, or secrets, using basic keywords such as 'password', or 'secret'."
...
"HARDEN_APPLOCKER",               "Invoke-AppLockerCheck",                      "TA0005 - Defense Evasion",      "AppLocker configuration",             "Low",      "List",   "True",     "True",       "False",        "Check whether an AppLocker policy is enforced."
"HARDEN_APPLOCKER_POLICY",        "Invoke-AppLockerPolicyCheck",                "TA0005 - Defense Evasion",      "AppLocker policy",                    "Low",      "List",   "True",     "False",      "False",        "Check whether AppLocker rules can be exploited, in the context of the current user, to execute arbitrary code."
"HARDEN_FILE_EXTENSION_ASSOC",    "Invoke-FileExtensionAssociationsCheck",      "TA0001 - Initial Access",       "File extension associations",         "Low",      "Table",  "True",     "True",       "False",        "Check whether file extensions such as '.bat' or '.wsh' are associated to a text editor. Note that only basic text editors such as 'Notepad' are detected. If a rich text editor is set instead, this check could yield false positives."
"HARDEN_HIDDEN_FILE_EXTENSIONS",  "Invoke-HiddenFilenameExtensionsCheck",       "TA0001 - Initial Access",       "Hidden file name extensions",         "Low",      "List",   "True",     "True",       "False",        "Check whether extensions of known file types are shown or hidden in the Explorer."
...
"CONFIG_SCCM_CACHE_FOLDERS",      "Invoke-SccmCacheFoldersCheck",               "TA0043 - Reconnaissance",       "SCCM cache folders",                  "None",     "List",   "True",     "True",       "False",        "Get information about SCCM cache folders (incl. number and list of binary, script, and text files)."
"CONFIG_PROXY_AUTO_CONFIG_URL",   "Invoke-ProxyAutoConfigUrlCheck",             "TA0001 - Initial Access",       "Proxy auto config URL",               "Low",      "List",   "True",     "True",       "False",        "Check whether a proxy auto config URL is set, and whether the proxy.pac file is retrieved over HTTP."
...
"UPDATE_HOTFIX_INFO",             "Invoke-HotFixCheck -Info",                   "TA0004 - Privilege Escalation", "Windows Update history",              "None",     "Table",  "True",     "True",       "False",        "Get information about Windows Update history. Update packages are sorted by date in descending order, so that most recent ones are shown first. Note that the script might fail to retrieve install dates when run with PowerShell version 2."
